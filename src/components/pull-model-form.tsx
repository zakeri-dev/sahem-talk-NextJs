...
export default function PullModelForm() {
  ...
  const handlePullModel = async (data: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast.error(`Error: ${error instanceof Error ? error.message : 'عدم موفقیت در دانلود سخنگوی جدید'}`)
    } finally {
      stopDownload()
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <p className='text-xs pt-1'>
                ...
                <a href='https://ollama.com/library' target='_blank' className='text-blue-500 underline'>
                  کتابخانه
                </a>{' '}
                را بررسی کنید{' '}
              </p>
